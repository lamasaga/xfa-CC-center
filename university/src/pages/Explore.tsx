...
type RankingType = 'qs' | 'us_news' | 'the';
type SortOption =
  | 'qs_asc'
  | 'qs_desc'
  | 'tuition_asc'
...
export default function Explore() {
  ...
  /* Ranking type is selected but not directly used in filtering yet — it affects display ranking label */
  void rankingType;
    ...
    if (!selectedRegions.includes('all')) {
      ...
      final.sort((a, b) => {
        switch (sortBy) {
          ...
          case 'salary_desc': {
            const ra = a.ranking?.qs ?? 99999;
            const rb = b.ranking?.qs ?? 99999;
            return ra - rb;
          }
          case 'name_asc':
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          />
          <p className="mt-4 text-[14px]" style={{ color: '#6B6560' }}>
            加载院校数据中...
          </p>
        </div>
      </div>
  ...
  return (
    <div
      className="fixed inset-0 z-[1200] overflow-hidden grid"
      style={{ backgroundColor: '#FAF7F2', gridTemplateRows: 'auto auto 1fr' }}
    >
      {/* ====== Header / Filter Bar ====== */}
      <div
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <h1
            className="font-space text-[28px] sm:text-[36px] font-normal tracking-tight shrink-0"
            style={{ color: '#2C2420' }}
          >
            院校库
          </h1>
          ...
          <div className="flex items-center gap-3 flex-1 sm:justify-end">
            ...
            >
              <SlidersHorizontal size={20} style={{ color: '#6B6560' }} />
            </button>
          </div>
        </div>
        ...
        >
          ...
          <div
            className="flex rounded-lg p-[3px] gap-[3px]"
            style={{ backgroundColor: '#F0EBE3' }}
          >
            {RANKING_TYPES.map((rt) => (
              <button
                key={rt.key}
                onClick={() => setRankingType(rt.key)}
                className="text-[13px] px-3 py-1.5 rounded-md transition-all duration-200"
                style={{
                  backgroundColor: rankingType === rt.key ? '#F5F0E8' : 'transparent',
                  color: rankingType === rt.key ? '#2C2420' : '#6B6560',
                  boxShadow: rankingType === rt.key ? '0 1px 3px rgba(44, 36, 32, 0.06)' : 'none',
                }}
              >
                {rt.label}
              </button>
        ...
        {activeFilterCount > 0 && (
          ...
          >
            ...
            <button
              ...
              className="text-[12px] transition-colors duration-200 hover:underline"
              style={{ color: '#C8A45C' }}
            >
              清除全部
            </button>
      ...
      <div
        ...
        style={{ padding: '24px max(24px, 2vw)', overscrollBehaviorY: 'contain' }}
      >
        {filteredUniversities.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <button
              onClick={clearAllFilters}
              className="text-[15px] mt-4 transition-colors duration-200 hover:underline"
              style={{ color: '#C8A45C' }}
            >
              清除所有筛选
            </button>
